...
public class RenderTextureCamera : MonoBehaviour
{
    ...
    IEnumerator StartRenderingToTexture()
    {
        ...
        if (SetMarkerBackground)
            GetComponent<Renderer> ().material.SetTexture ("_BackTex", SetMarkerBackground);
        if (MarkerBackgroundAlpha)
            GetComponent<Renderer> ().material.SetFloat ("_AlphaBack", 0);
        else
            GetComponent<Renderer> ().material.SetFloat ("_AlphaBack", 1);
    ...
    void OnValidate()
    {
        ...
        {
            if (GetComponent<RenderTextureCamera> ().isActiveAndEnabled)
                StartCoroutine (ShowTextureOnGUI ());
        }
        #endif
