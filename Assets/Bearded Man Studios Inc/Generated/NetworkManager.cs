...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DuckNetworkObject = null;
		public GameObject[] DuckControllerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DuckNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DuckNetworkObject.Length > 0 && DuckNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DuckNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is DuckControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DuckControllerNetworkObject.Length > 0 && DuckControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DuckControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDuck instead, its shorter and easier to type out ;)")]
		public DuckBehavior InstantiateDuckNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DuckNetworkObject[index]);
			var netBehavior = go.GetComponent<DuckBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DuckBehavior>().networkObject = (DuckNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateDuckController instead, its shorter and easier to type out ;)")]
		public DuckControllerBehavior InstantiateDuckControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DuckControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<DuckControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DuckControllerBehavior>().networkObject = (DuckControllerNetworkObject)obj;
		...
		public DuckBehavior InstantiateDuck(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DuckNetworkObject[index]);
			var netBehavior = go.GetComponent<DuckBehavior>();
		...
		public DuckControllerBehavior InstantiateDuckController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DuckControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<DuckControllerBehavior>();
